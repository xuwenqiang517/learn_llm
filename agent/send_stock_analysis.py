...
def get_latest_concept_table(temp_dir: str = "/Users/JDb/Desktop/github/learn_llm/.temp") -> Optional[Path]:
    """
    获取最新的概念分析表格文件（从 output/tools/ 目录）

    Args:
        temp_dir: .temp 目录路径

    Returns:
        最新概念分析表格文件的 Path 对象，如果没有找到返回 None
    """
    tools_dir = Path(temp_dir) / "output" / "tools"
    if not tools_dir.exists():
        logger.error(f"目录不存在: {tools_dir}")
    ...
    table_files = list(tools_dir.glob("concept_analysis_*.md"))
    if not table_files:
        logger.warning("未找到概念分析表格文件")
        return None

    latest_file = max(table_files, key=lambda f: f.stat().st_mtime)
    logger.info(f"找到最新概念分析表格: {latest_file}")
    return latest_file
...
def send_rising_stocks_table(
    ...
    """
    try:
        logger.info("开始发送连续上涨股票表格到飞书...")

        table_file = get_latest_table_file(temp_dir)
        if not table_file:
            logger.error("未找到连续上涨股票表格文件")
            return False

        today = datetime.now().strftime("%Y年%m月%d日")
        title = f"📈 连续上涨股票 - {today}"
        
        table_content = table_file.read_text(encoding="utf-8")
        
        bot = FeishuBot(webhook_url)

        if not bot.send_markdown(title, table_content):
            logger.error("连续上涨股票表格发送失败")
            return False

        logger.info(f"已发送连续上涨股票表格: {table_file.name}")
        return True

    except Exception as e:
        logger.error(f"发送连续上涨股票表格异常: {e}")
        return False


def send_concept_analysis_table(
    webhook_url: str = "https://open.feishu.cn/open-apis/bot/v2/hook/c8278f54-8e18-4edc-97bd-0c0abc3ab17f",
    temp_dir: str = "/Users/JDb/Desktop/github/learn_llm/.temp",
    ...
    """
    try:
        logger.info("开始发送概念分析表格到飞书...")

        table_file = get_latest_concept_table(temp_dir)
        if not table_file:
            logger.error("未找到概念分析表格文件")
            return False
        ...
        if not bot.send_markdown(title, table_content):
            logger.error("概念分析表格发送失败")
            return False

        logger.info(f"已发送概念分析表格: {table_file.name}")
        return True

    except Exception as e:
        logger.error(f"发送概念分析表格异常: {e}")
        return False

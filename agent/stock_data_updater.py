...
def _update_stock_list(force_update: bool = False) -> bool:
    ...
    Returns:
        是否更新成功
    """
    if not force_update and FileUtil.is_cache_valid(STOCK_LIST_FILE, CACHE_EXPIRY_DAYS):
        cached = JsonUtil.load(STOCK_LIST_FILE)
...
def _update_industry_map(force_update: bool = False) -> bool:
    ...
    Returns:
        是否更新成功
    """
    if not force_update and FileUtil.is_cache_valid(INDUSTRY_MAP_FILE, CACHE_EXPIRY_DAYS):
        cached = JsonUtil.load(INDUSTRY_MAP_FILE)
        if cached is not None:
            return True
...
def _update_concept_map(force_update: bool = False) -> bool:
    ...
    Returns:
        是否更新成功
    """
    if not force_update and FileUtil.is_cache_valid(CONCEPT_MAP_FILE, CACHE_EXPIRY_DAYS):
        cached = JsonUtil.load(CONCEPT_MAP_FILE)
        if cached is not None:
            return True

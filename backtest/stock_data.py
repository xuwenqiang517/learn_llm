...
def pick_rule(info: StockDayData) -> bool:
        ...
        if info.volume_trend==True and info.consecutive_up_days>=2 and info.change_pct < 9.8:
            return True
...
def pick_stock(dict: Dict[str, Dict[str, StockDayData]], day: str,max_count: int = 5) -> list:
    ...
    for code, data in dict.items():
        ...
        if pick_rule(info):
            rs_list.append(info)
    # 按涨幅最小排序
    rs_list.sort(key=lambda x: x.pe, reverse=False)
...
def back_test(start:str="20260101",end:str="20260119",data_dict:Dict[str, Dict[str, StockDayData]]=None,base_total_amount:float=100000.00,buy_count:int=10000,max_hold_count:int=10,print_detail:bool=False,print_fail_reason:bool=False)->tuple[float,float]:
    ...
    while cur_day<end:
        ...
        for stock in stock_list:
            print_blue("pick:" + stock.full_info(),print_detail)
        # 判断卖出
        ...
        print_white(f"{cur_day} 空闲金额:{total_free_amount} 持仓金额:{total_hold_amount} 总金额:{total_free_amount+total_hold_amount}",print_detail)
        # 更新最高收益率
...
def test_1():
    ...
    back_test(data_dict=data_dict,start=start_date,end=end_date,max_hold_count=5,buy_count=20000,base_total_amount=100000.00,print_detail=True)
